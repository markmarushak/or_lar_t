    ...
    protected function renderAllSubmittedDataHtml($showEmptyFields)
    {
        ...
        foreach ($this->form->getRecursiveIterator(RecursiveIteratorIterator::SELF_FIRST) as $element) {
            ...
            if ($element instanceof Quform_Element_Html) {
                ...
                if ($element->config('showLabelInEmail') && Quform::isNonEmptyString($element->config('label'))) {
                    $content .= '<tr><td valign="top" style="font-family: Helvetica, Arial, sans-serif; font-size: 22px; font-weight: bold; background-color: #c73412; color: #ffffff; border-bottom: 1px solid #e14e2c; word-wrap: break-word;">' . esc_html($element->config('label')) . '</td></tr>' . self::NEWLINE;
                }
            }  else if ($element instanceof Quform_Element_Group) {
                if ($element->config('showLabelInEmail') && Quform::isNonEmptyString($element->config('label'))) {
                    $content .= '<tr><td valign="top" style="font-family: Helvetica, Arial, sans-serif; font-size: 17px; background-color: #c73412; color: #ffffff; word-wrap: break-word;">' . esc_html($element->config('label')) . '</td></tr>' . self::NEWLINE;
                }
            } else if ($element instanceof Quform_Element_Field) {
                if ($element->config('showInEmail')) {
