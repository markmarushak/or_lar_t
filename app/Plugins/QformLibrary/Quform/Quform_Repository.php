...
class Quform_Repository
{
    ...
    public function all($active = null)
    {
        ...
        $sql = "SELECT * FROM " . $this->getFormsTableName() . " WHERE trashed = 0";
    ...
    public function getForms(array $args = array())
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS f.id, f.name, f.active, f.trashed, f.updated_at,
                ...
                LEFT JOIN ( SELECT form_id, COUNT(*) AS cnt FROM " . $this->getEntriesTableName() . " GROUP BY form_id ) e
                ON f.id = e.form_id
     ...
     * @return  array
     */
    public function formsToSelectArray($active = null)
    {
        $rows = $this->all($active);
        $forms = array();
     ...
     * @param   int    $entryId  The entry ID if we are updating a saved entry, 0 for new entries
     * @return  array            The entry config with new ID if we are adding a new entry
     */
    public function saveEntry(array $config, $entryId = 0)
    {
        global $wpdb;
        ...
        if ($entryId == 0 || ! $this->entryExists($entryId)) {
...
GROUP BY entries.id";
        ...
        // Search clause
        if (Quform::isNonEmptyString($args['search']) || count($args['labels'])) {
            $sql .= " HAVING ";
            $having = array();
    ...
    public function setFormEntryLabels($formId, array $labels)
    {
        ...
        if ($ids == '') {
            ...
            $this->deleteFormEntryLabels($formId);
        } else {
            // Some old IDs - delete those not in the list
            $wpdb->query("DELETE FROM " . $this->getEntryEntryLabelsTableName() . " WHERE entry_label_id NOT IN ($ids)");
            $wpdb->query("DELETE FROM " . $this->getEntryLabelsTableName() . " WHERE id NOT IN ($ids)");
        }
    ...
    public function deleteFormEntryLabels($formId)
    {
        ...
        $wpdb->query($wpdb->prepare("DELETE FROM " . $this->getEntryEntryLabelsTableName() . " WHERE entry_label_id IN (SELECT id FROM " . $this->getEntryLabelsTableName() . " WHERE form_id = %d)", $formId));
        $wpdb->query($wpdb->prepare("DELETE FROM " . $this->getEntryLabelsTableName() . " WHERE form_id = %d", $formId));
    }
...
GROUP BY `data`.`entry_id`", $entryId);
    ...
    public function hasDuplicateEntry(Quform_Element_Field $element)
    {
        ...
        $sql = "SELECT `e`.`id` FROM `" . $this->getEntryDataTableName() . "` ed LEFT JOIN `" .
...
AND `ed`.`value` = '%s';";
    ...
    public function getAllFormsWithUnreadEntries()
    {
        ...
        $sql = "SELECT f.id, f.name, (SELECT COUNT(*) FROM " . $this->getEntriesTableName() . " WHERE form_id = f.id AND unread = 1) AS entries FROM " . $this->getFormsTableName() . " f HAVING entries > 0;";
    ...
    public function getRecentEntries($count = null)
    {
        ...
        $sql = "SELECT f.name, e.* FROM " . $this->getEntriesTableName() . " e LEFT JOIN " . $this->getFormsTableName() . " f ON e.form_id = f.id ORDER BY e.created_at DESC";
    ...
    public function exportEntries(Quform_Form $form, $from = '', $to = '')
    {
        ...
        $sql .= $wpdb->prepare(" FROM `" . $this->getEntriesTableName() . "` `entries`
LEFT JOIN `" . $this->getEntryDataTableName() . "` `data` ON `data`.`entry_id` = `entries`.`id`
WHERE `entries`.`form_id` = %d",$form->getId());
