...
class Quform_ScriptLoader
{
     ...
     */
    public function __construct(Quform_Options $options, Quform_Themes $themes, Quform_Repository $repository, Quform_Form_Factory $factory) {
    ...
    protected function enqueueScripts()
    {
        ...
        wp_localize_script('quform', 'quformL10n', $this->jsL10n());
    }
    ...
    {
        echo "<script>(function(h,c){h[c]=h[c]+(h[c]&&' ')+'quform-js'})(document.documentElement,'className');</script>";
    }
    ...
    {
        if (wp_script_is('quform-recaptcha', 'registered')) {
            wp_print_scripts('quform-recaptcha');
        }
    }
    ...
    protected function generateFormCssFile($forms)
    {
        ...
        foreach ($forms as $form) {
            ...
            if ($css) {
                ...
                if ($result === false) {
                    Quform::debug('Could not generate custom CSS file: failed to save form CSS file');
                }
    ...
    protected function generateCustomCssFile()
    {
        ...
        if ($combined != '') {
            $result = @file_put_contents($combinedPath, $combined);

            if ($result === false) {
                Quform::debug('Could not generate custom CSS file: failed to save combined CSS file');
            }
        } else {
    ...
    protected function generateCustomJsFile()
    {
        ...
        if ($contents != '') {
            ...
            if ($result === false) {
                Quform::debug('Could not generate custom JS file: failed to save combined JS file');
            }
        } else {
            if (is_file($path)) {
                @unlink($path);
