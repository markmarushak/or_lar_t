...
abstract class Quform_Element_Field extends Quform_Element
{
    ...
    public function render(array $context = array())
    {
        ...
        if ($this->isVisible()) {
            ...
            $output .= $this->getDescriptionHtml('above');
    ...
    protected function getLabelHtml(array $context = array(), $forAttribute = true, $id = false)
    {
        ...
        $output .= sprintf(
            '<label class="quform-label-text"%s%s>%s</label>',
            ($id ? ' id="' . Quform::escape($this->getUniqueId() . '_label') . '"' : ''),
            (isset($forValue) ? ' for="' . Quform::escape($forValue) . '"' : ''),
            $this->config('label')
        );

        if ($this->isRequired() && Quform::isNonEmptyString($requiredText = $this->form->config('requiredText'))) {
            $output .= '<span class="quform-required">' . esc_html($requiredText) . '</span>';
        }
    ...
    {
        return Quform::isNonEmptyString($this->config('adminLabel')) ? $this->config('adminLabel') : $this->config('label');
    }
    ...
    {
        return Quform::isNonEmptyString($this->error);
    }
    ...
    /**
     * Clear the current validation error message
     */
     ...
     * @return string
     */
    protected function getSubLabelHtml()
    {
        $output = '';
        $subLabel = $this->config('subLabel');
        ...
        $output = sprintf(
            '<label id="%s_sub_label" class="quform-sub-label">%s</label>',
    ...
    protected function renderCss(array $context = array())
    {
        ...
        if ($context['labelPosition'] == 'left' && $context['labelWidth'] != '150px') {
            $css .= sprintf('.quform-labels-left.quform-element-%1$s > .quform-spacer > .quform-label, .quform-rtl .quform-labels-left.quform-element-%1$s > .quform-spacer > .quform-inner { width: %2$s; }', $this->getIdentifier(), $context['labelWidth']);
            $css .= sprintf('.quform-labels-left.quform-element-%1$s > .quform-spacer > .quform-inner { margin-left: %2$s; }', $this->getIdentifier(), $context['labelWidth']);
            $css .= sprintf('.quform-rtl .quform-labels-left.quform-element-%1$s > .quform-spacer > .quform-inner { margin-right: %2$s; margin-left: 0; }', $this->getIdentifier(), $context['labelWidth']);
        }
    ...
    protected function getCssSelectors()
    {
        return array(
            ...
            'elementRequiredText' => '%s .quform-label-%s > .quform-required',
    ...
    protected function isLogicValueMatch($value, array $rule)
    {
        switch ($rule['operator']) {
            ...
            case 'not_empty':
                return $value !== $this->getEmptyValue();
            case 'gt':
                return is_numeric($value) && $value > $rule['value'];
            case 'lt':
                return is_numeric($value) && $value < $rule['value'];
            case 'contains':
