...
class Quform_Form extends Model
{
     ...
     */
    public function __construct($config, $id  = null, $uniqueId  = null, Quform_Session $session  = null, Quform_TokenReplacer $tokenReplacer = null, Quform_Options $options = null)
    {
    ...
    public function render(array $options = array())
        {
            ...
            $output .= sprintf('<div id="quform-%s" class="%s">',
                                '5206b2',
                                'quform quform-1');
            ...
            $output .= sprintf('<div class="quform-form-inner quform-form-inner-%d">', 1);
    ...
    public function getJsConfig()
    {
        $config = array(
            ...
            'updateFancybox' => 'quform_update_fancybox',
    ...
    public function getCss()
    {
        ...
        $map = array(
            ...
            array('labelTextColor', 'color: %s;', 'elementLabel'),
        ...
        if ($this->config('loadingOverlay') && Quform::isNonEmptyString($this->config('loadingOverlayColor'))) {
            $css .= sprintf(
                '.quform-%1$d .quform-loading-overlay { background-color: %2$s; }',
                $this->getId(),
    ...
    protected function getCssSelectors()
    {
        return array(
            ...
            'elementRequiredText' => '%s .quform-label > .quform-required',
            ...
            'groupTitle' => '%s .quform-group-title',
            'groupDescription' => '%s .quform-group-description',
    ...
    public function calculateElementVisibility()
    {
        $parentGroups = array();
        $currentPage = null;
        $this->calculateElementVisibilityHelper($this->pages, $parentGroups, $currentPage);
    }
     ...
     */
    protected function calculateElementVisibilityHelper($elements, &$parentGroups, &$currentPage)
    {
        foreach ($elements as $element) {
            ...
            if ( ! $isConditionallyHidden && $element->config('logicEnabled') && $ruleCount = count($element->config('logicRules'))) {
            ...
            if ($element instanceof Quform_Element_Group || $element instanceof Quform_Element_Row || $element instanceof Quform_Element_Column) {
                ...
                $this->calculateElementVisibilityHelper($element->getElements(), $parentGroups, $currentPage);
