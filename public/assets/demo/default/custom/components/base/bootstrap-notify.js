var BootstrapNotifyDemo = function () {
    var e = function () {
        $("[data-switch=true]").bootstrapSwitch({size: 'mini'}), $("#m_notify_btn").click(function () {
            var e = {};
            e.message = "New order has been placed", $("#m_notify_title").prop("checked") && (e.title = "Notification Title"), "" != $("#m_notify_icon").val() && (e.icon = "icon " + $("#m_notify_icon").val()), $("#m_notify_url").prop("checked") && (e.url = "www.keenthemes.com", e.target = "_blank");
            var t = $.notify(e, {
                ...
                placement: {from: $("#m_notify_placement_from").val(), align: $("#m_notify_placement_align").val()},
                offset: {x: $("#m_notify_offset_x").val(), y: $("#m_notify_offset_y").val()},
